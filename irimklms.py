mainfrom shogun.Features import CombinedFeatures, RealFeatures, Labels
...
DATA_SOURCE_1 = "data/iri/da.%s"
DATA_SOURCE_2 = "data/iri/go.%s"
DATA_SOURCE_3 = "data/iri/ia.%s"
DATA_SOURCE_4 = "data/iri/ip.%s"
DATA_SOURCE_5 = "data/iri/oa.%s"
...
def load_iri_data(data_source):
	data = file(data_source % "feats").read().split("\n")
	data = map(clean, data)
	labs = array(map(lambda x : 1 if x == 1 else -1, map(int, file(data_source % "labs").read().strip().split("\n"))))
...
data_4, labs_4 = load_iri_data(DATA_SOURCE_4)
data_5, labs_5 = load_iri_data(DATA_SOURCE_5)
