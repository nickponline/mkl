...
dist=0.5
D=500
traindata_real = concatenate((randn(2,D)-dist, randn(2,D)+dist), axis=1)
testdata_real = concatenate((randn(2,D)-dist, randn(2,D)+dist), axis=1)
train_labels = concatenate((-ones(D), ones(D)))
test_labels = concatenate((-ones(D), ones(D)))

# Pack labels and features into desnse representation
feats_train = SparseRealFeatures(traindata_real);
feats_test = SparseRealFeatures(testdata_real);
#feats_test  = SparseRealFeatures(testdata_real);
train_labels      = BinaryLabels(train_labels);
test_labels = BinaryLabels(test_labels)
...
splitting_strategy=StratifiedCrossValidationSplitting(train_labels, 15)
evaluation_criterium=ContingencyTableEvaluation(ACCURACY)
cross_validation=CrossValidation(classifier, feats_train, train_labels, splitting_strategy, evaluation_criterium)
